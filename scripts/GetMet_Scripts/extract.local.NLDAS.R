##' Extract a single NLDAS grid cell from files that have been downloaded, stored locally, and converted from grib to netcdf format
##' @name extract.local.NLDAS
...
download.NLDAS <- function(outfolder, start_date, end_date, site_id, lat.in, lon.in, dir.nldas="/Volumes/Celtis/Meteorology/LDAS/NLDAS_FORA0125_H.002/netcdf/", overwrite=FALSE, verbose=FALSE, ...){  
  # require(PEcAn.utils)
  # require(RCurl)
  require(lubridate)
  require(ncdf4)
  ...
  for (i in 1:rows){
    ...
    for(j in 1:length(days.use)){
      ...
      dap_file <- nc_open(file.path(dir.nldas,y.now,mo.now,paste0("NLDAS_FORA0125_H.A",y.now, mo.now,day.mo,".nc")))
    ...
    # results$host[i] <- fqdn()
    # results$startdate[i] <- paste0(y.now,"-01-01 00:00:00")
    # results$enddate[i] <- paste0(y.now,"-12-31 23:59:59")
