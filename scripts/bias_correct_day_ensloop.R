...
# for(v in 1:7){
  ...
  for(i in 1:nrow(yrs.cal)){ # Working through one dataset at a time
    ...
    # Note: Now doing bias correction by looping though ensemble members, this will almost 
    #       certianly slow things down, but will hopefully fix some issues with needing 
    #       massive amounts of memory, particularly for large ensembles    
	...
	for(ens in 1:n){
		...
		#    an anomaly
		# ---------
		# mod.bias2 <- gam(Y ~ s(doy, k=12) + X, data=dat.temp[dat.temp$ind == ind, ])
		mod.bias <- gam(Y ~ s(doy, k=6) + X, data=dat.temp[dat.temp$ind == ind, ])
		summary(mod.bias)
	
		# Saving the mean predicted & residuals
		dat.temp[dat.temp$ind == ind, "pred"]  <- predict(mod.bias)
		dat.temp[dat.temp$ind == ind, "resid"] <- resid(mod.bias)
		summary(dat.temp)
		...
		# hist(dat.temp$resid)
		dat.pred[dat.pred$ind == ind, "pred"] <- predict(mod.bias, newdata=dat.pred[dat.pred$ind == ind, ])
		...
		raw.train[raw.train$ind==ind, "pred"] <- predict(mod.bias, newdata=raw.train[raw.train$ind==ind,])
		raw.bias[raw.bias$ind==ind, "pred"] <- predict(mod.bias, newdata=raw.bias[raw.bias$ind==ind,])
		...
		anom.train <- gam(X ~ s(doy, k=6) , data=raw.train[raw.train$ind==ind,]) # Need to account for the climatic differences in the simulations 
		anom.bias  <- gam(X ~ s(doy, k=6) , data=raw.bias[raw.bias$ind==ind,]) # Note: the "bias" dataset has not corrected yet, so there should only be no variation around the values

		raw.train[raw.train$ind==ind,"anom.train"] <- resid(anom.train)
		raw.bias [raw.bias$ind==ind, "anom.raw"  ] <- resid(anom.bias)      
		dat.pred[dat.pred$ind==ind, "anom.raw"] <- dat.pred[dat.pred$ind==ind, "X"] - predict(anom.bias, newdata=dat.pred[dat.pred$ind==ind, ])
		# par(mfrow=c(2,1))
		...
		for(j in vars.met[vars.met!=met.var]){
		  # print(j)
		  raw.train[raw.train$ind==ind, "Q"] <- raw.train[raw.train$ind==ind,j]
		  raw.bias[raw.bias$ind==ind, "Q"] <- raw.bias[raw.bias$ind==ind,j]
		  dat.pred[dat.pred$ind==ind, "Q"] <- dat.pred[dat.pred$ind==ind,j]
	  
		  anom.train2  <- gam(Q ~ s(doy, k=6), data=raw.train[raw.train$ind==ind,]) # Need to account for the climatic differences in the simulations 
		  anom.bias2  <- gam(Q ~ s(doy, k=6) , data=raw.bias[raw.bias$ind==ind,]) # Note: the "bias" dataset has not been corrected yet, so there should only be 1 value
	  
		  raw.train[raw.train$ind==ind, paste0(j, ".anom")] <- resid(anom.train2)
		  raw.bias[raw.bias$ind==ind, paste0(j, ".anom")] <- resid(anom.bias2)
		  dat.pred[dat.pred$ind==ind, paste0(j, ".anom")] <- dat.pred[dat.pred$ind==ind,"Q"] - predict(anom.bias2, newdata=dat.pred[dat.pred$ind==ind,])
		...
		if(dat.bias %in% empirical & dat.train %in% empirical){ 
		  ...
		  dat.anom <- merge(raw.bias[raw.bias$ind==ind,c("year", "doy", "ind", "X", "anom.raw")], 
		                    raw.train[raw.train$ind==ind,c("year", "doy", "anom.train", "ind", vars.met[vars.met!=met.var], paste0(vars.met[vars.met!=met.var], ".anom"))])
		  ...
		  if (met.var %in% c("tair", "tmax", "tmin")){
  			...
  			mod.anom <- gam(anom.raw ~ s(doy, k=6) + s(year, k=k) + tmax.anom*tmin.anom -1 , data=dat.pred[dat.pred$ind==ind,])
  			...
  			mod.anom <- gam(anom.raw ~ s(year, k=k) + (tmax.anom + tmin.anom + swdown.anom + lwdown.anom + qair.anom) -1, data=dat.pred[dat.pred$ind==ind,])
  			...
  			mod.anom <- gam(anom.raw ~ s(doy, k=6) + (tmin*tmax.anom + swdown.anom + qair.anom) -1, data=dat.pred[dat.pred$ind==ind,])
		  }      
		} else { 
		  ...
		  if(met.var %in% c("tmax", "tmin")){
  			mod.anom <- gam(anom.raw ~ s(year, k=k) -1, data=dat.pred[dat.pred$ind==ind,]) # Because all will be the same
		  } else if(met.var=="precipf"){ 
  			...
  			mod.anom <- gam(anom.train ~ s(doy, k=4) + (tmax.anom*tmin.anom + qair.anom + press.anom + wind.anom) -1, data=raw.train[raw.train$ind==ind,])
		  } else {
		...
		Xp <- predict(mod.bias, newdata=dat.pred[dat.pred$ind==ind,], type="lpmatrix")
		Xp.anom <- predict(mod.anom, newdata=dat.pred[dat.pred$ind==ind,], type="lpmatrix")
		if(met.var == "precipf"){
		  # Linear models have a bit of a difference
		  # Xp.ann <- predict(mod.ann, newdata=dat.pred, type="lpmatrix")
	  
		  dat.pred[dat.pred$ind==ind,"Y.tot"] <- dat.pred[dat.pred$ind==ind,"pred.ann"]
		  mod.terms <- terms(mod.ann)
		  m <- model.frame(mod.terms, dat.pred[dat.pred$ind==ind,], xlev=mod.ann$xlevels)
		...
		if(met.var %in% c("swdown", "qair", "lwdown", "wind")){
		  sim1 <- sim1^2
		  dat.pred[dat.pred$ind==ind,"X"] <- dat.pred[dat.pred$ind==ind,"X"]^2
		} 
		...
		if(met.var=="precipf"){
		  for(j in 1:ncol(sim1)){
			for(y in min(dat.pred[dat.pred$ind==ind, "year"]):max(dat.pred[dat.pred$ind==ind, "year"])){
			  # Figure out which rows belong to this particular year
			  rows.yr <- which(dat.pred[dat.pred$ind==ind, "year"]==y)
		...
		sim.final[,ens] <- sim1[,sample(1:n,1)]
