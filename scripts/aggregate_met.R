...
aggregate.met <- function(path.in, years.agg, save.day=T, save.month=T, 
                          ...
                          parallel=F, n.cores=NULL, verbose=FALSE){
  
  # Create our output folders
  dir.create(file.path(out.base, day.dir), recursive=T, showWarnings = FALSE)
  dir.create(file.path(out.base, mo.dir), recursive=T, showWarnings = FALSE)
  ...
  tictoc::tic()
  pb <- txtProgressBar(min=0, max=length(files.agg), style=3)
  for(i in 1:length(files.agg)){
    yr.now <- strsplit(files.agg, "[.]")[[1]]
    yr.now <- as.numeric(yr.now[length(yr.now)-1])
    nday <- ifelse(lubridate::leap_year(yr.now), 366, 365)
    
    # Open the file
    ncT <- ncdf4::nc_open(file.path(path.in, files.agg[i]))
    ...
    if(save.day==TRUE){
      ...
      for(j in vars.file){
        var.list[[j]] <- ncdf4::ncvar_def(name = j,
                                          units = ncT$var[[v]]$units, dim = nc.dim, missval = -9999,
                                          verbose = verbose)
      }
      ...
      loc <- ncdf4::nc_create(filename = file.path(out.base, day.dir, files.agg[i]), vars = var.list, verbose = verbose)
    ...
    if(save.month==TRUE){
      ...
      loc <- ncdf4::nc_create(filename = file.path(out.base, mo.dir, files.agg[i]), vars = var.list, verbose = verbose)
