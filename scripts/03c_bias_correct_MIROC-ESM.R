...
# for(GCM in GCM.list){
  ...
  # Might want to parse down the years for yrs.train... doing the full time series could maybe throw things off if they don't
  # get the recent warming right
  met.out <- align.met(train.path, source.path, yrs.train=1850:1900, yrs.source=850:1849, n.ens=n.ens, seed=201708, pair.mems = FALSE, mems.train=paste(ens.ID, ens.mems, sep="_"))
